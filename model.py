...
def build_token_to_id_vocab(sentences, specials=('<pad>', '<bos>', '<eos>', '<unk>')):
    ...
    result = {}
    sp_tokens = list(specials)
    words = [word for sentence in sentences for word in sentence.split()]
    unique_tokens = list(dict.fromkeys(sp_tokens + words))
...
def pad_id_sequence(ids, max_len, pad_id):
    ...
    id_len = len(ids)
    if id_len < max_len:
        return ids + [pad_id]*(max_len-id_len)
    elif id_len > max_len:
        return ids[:max_len]
    else:
        return ids
...
def build_position_index_column(max_len):
    ...
    indices = torch.arange(max_len, dtype=torch.float32)
...
def add_positional_encoding_to_embeddings(embedded_batch, positional_encoding):
    ...
    add_tensor = positional_encoding[:L, :]

    res = add_tensor+embedded_batch
...
def split_qkv_into_heads(q, k, v, num_heads):
    ...
    for i in [q, k, v]:
        ...
        tensor = transpose_heads_before_sequence(temp)
        res.append(tensor)
